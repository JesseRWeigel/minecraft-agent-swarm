...
class ProtectedWorkerTests(unittest.TestCase):
    ...
    def test_fault_injection_cannot_be_accepted_even_with_successful_samples(self):
        for case in ("death", "disconnect", "observer_timeout", "disk_full", "creative_mode", "command_denied", "command_authorized", "rcon_stall", "mid_action_disconnect", "unknown"):
            row = outcome(); row["failure_case"] = case
            self.assertFalse(validate_result(row, "forward"))

    def test_during_sample_cannot_replace_terminal_evidence(self):
        row = outcome()
        row["terminal"]["phase"] = "during"
        self.assertFalse(validate_result(row, "forward"))
