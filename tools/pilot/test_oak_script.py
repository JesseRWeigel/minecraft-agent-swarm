...
class ScriptedHostAcceptanceTests(unittest.TestCase):
 def test_missing_or_failed_action_receipt_cannot_be_promoted_by_successful_endpoint(self):
  ...
  for mutate in [lambda v:v.pop('action_script'),lambda v:v['action_script'].update(status='failed'),lambda v:v.update(action_output_eof=False),lambda v:v.pop('action_driver')]:
