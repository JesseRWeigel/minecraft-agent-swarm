...
An isolated scripted oak collection qualified with the change: seven requests, one oak log acquired, normal lifecycle and independent score. The initial same-source stationary attempt failed before game launch because the private storage helper exited 3 during mount. Its image and manifest remain preserved. A fresh stationary retry now passes with empty inventory, intact target, unchanged position and normal shutdown. Both qualified controls use identical captured source/controller, snapshot, server and dependency hashes. A read-only diagnostic mount of the failed image also succeeded and left its hash unchanged. The original write-mount failure remains unexplained; the passing retry does not erase it. No model inference or live-swarm changes were made.
