...
mode() { jq -r .mode "$STATE" 2>/dev/null || echo live; }
case "${1:-status}" in
  status)
  ...
  enter-maintenance)
    ...
    jq -n --arg m maintenance --arg s "$(now)" --arg r "$reason" --arg b "${OPS_BY:-Claude (operator)}" \
      '{mode:$m,since:$s,reason:$r,by:$b,trial:null}' > "$STATE"
  ...
  exit-maintenance)
    jq -n --arg m live --arg s "$(now)" --arg b "${OPS_BY:-Claude (operator)}" '{mode:$m,since:$s,reason:"live campaign (observational)",by:$b,trial:null}' > "$STATE"
  ...
  enter-evaluation)
    ...
    jq -n --arg m evaluation --arg s "$(now)" --arg r "$note" --arg t "$trial" --arg b "${OPS_BY:-Claude (operator)}" '{mode:$m,since:$s,reason:$r,by:$b,trial:$t}' > "$STATE"
